...
function renderError(message, back="/"){
  ...
  <style>
    ...
    .glass-card { background: rgba(255,255,255,0.02); padding:24px 36px; border-radius:18px; border:1px solid rgba(56,189,248,0.22); text-align:center; }
...
// Deposit / MoneyFusion Activation
app.post("/deposit", requireAuth, async (req,res)=>{
  try {
    ...
    payments.push({id:paymentId,user:user.username,amount:Number(amount),status:"pending",duration: Math.round(amount/800*24)});
...
app.post("/webhook",(req,res)=>{
  ...
  // Créditer bot : convertir FCFA -> heures bot
  const duration = pay.duration || 24; 
  const now = Date.now();
  const previous = user.botActiveUntil>now?user.botActiveUntil:now;
  user.botActiveUntil = previous + duration*3600*1000;
